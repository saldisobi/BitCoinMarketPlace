...
import com.saldi.domain.repository.TestBitcoinMarketPriceSourceRemote
 ...
 */
class RetrieveBitcoinMarketPriceUseCaseTest {
    ...
    class RetrieveTransactionsUseCaseTest {
        ...
        fun setup() {
            val testBitcoinRepository = BitcoinRepositoryImpl(
                TestBitcoinMarketPriceSourceRemote()
            )
        ...
        fun `returns market price chart data`() = coroutineRule.runBlockingTest {
            ...
            MatcherAssert.assertThat(
                result.data,
                Is.`is`(CoreMatchers.equalTo(TestData.marketPriceData))
            )
        }
