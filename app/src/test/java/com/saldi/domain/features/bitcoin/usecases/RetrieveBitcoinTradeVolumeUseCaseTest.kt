...
import com.saldi.domain.repository.TestBitcoinTradeVolumeSourceRemote
...
class RetrieveBitcoinTradeVolumeUseCaseTest {
    ...
    private lateinit var useCase: RetrieveBitcoinTradeVolumeUseCase

    @Before
    fun setup() {
        val testUserEventRepository = BitcoinRepositoryImpl(
            TestBitcoinTradeVolumeSourceRemote()
        )

        useCase = RetrieveBitcoinTradeVolumeUseCase(
    ...
    @Test
    fun `returns trade volume chart data`() = coroutineRule.runBlockingTest {
        val result = useCase(Unit).first { it is DomainResult.Success }
        MatcherAssert.assertThat(
            result.data,
            Is.`is`(CoreMatchers.equalTo(TestData.tradeVolumeData))
        )
    }
